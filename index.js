...
function addArtist(array, newName, newYears, newGenre, newNat, newBio) {
  const addedArtist = [...array];
  const newArtist = {
    name: newName,
    years: newYears,
    genre: newGenre,
  ...
  addedArtist.push(newArtist);
  return addedArtist;
};
addArtist(artists, "Kelsey", "1990-2022", "Full Stack Development", "White", "I have a background in financial operations at various locations. I am attending BloomTech so I can become a developer.")
...
function artistByCountry(array, artistNat){
  const natArray = [];
  for(let i = 0; i < array.length; i++) {
    if(array[i].nationality === artistNat) {
